...
Este projeto utiliza uma hierarquia de herança de classes para compartilhar a lógica de automação específica do Salesforce entre todos os page objects. Toda página de objeto (`AccountPage`, `AcomodacaoPage`, etc.) estende `BasePage`, que centraliza as operações comuns a todas as páginas do Salesforce Lightning.
...
// AcomodacaoPage — mesmo boilerplate duplicado
...
// AcomodacaoPage — mesma estrutura, zero duplicação
async createAcomodacao(name: string) {
  await this.clickNew();
...
Como os nomes dos métodos expressam intenção (`createAccount`, `deleteAcomodacao`) em vez de implementação (`getByRole(...).click()`), os specs se leem como fluxos estruturados de usuário:

```typescript
test("deve excluir uma conta", async ({ accountPage }) => {
  const name = `Hotel Teste ${Date.now()}`;
  await accountPage.createAccount(name);
  await accountPage.navigate();
  await accountPage.deleteAccount(name);
  await expect(accountPage.getListItem(name)).not.toBeVisible();
});
```
...
Abra `tests/fixtures/index.ts` e adicione o novo page object junto aos existentes:
